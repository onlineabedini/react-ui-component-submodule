"use client";
// ProfileImage: Displays and optionally edits a user's profile image
...
const ProfileImage: React.FC<ProfileImageProps> = ({
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      >
        {/* Optimized profile image with LazyImage */}
        <LazyImage
          src={imgError ? getDefaultProviderImage() : imageUrl}
          alt={alt || (username ? `${username}'s profile` : t("profileImageAlt"))}
          className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
          onLoad={() => {}}
          style={{}}
        />
        {/* Fallback: if LazyImage fails, set imgError */}
        {/* We can't use onError directly, so use a transparent overlay button to trigger error for demo */}
        {/* Optionally, you could extend LazyImage to support onError */}
        {editable && (
