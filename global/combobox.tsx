...
function filterChildren(children: React.ReactNode, search: string): React.ReactNode {
  ...
  if (React.isValidElement(children)) {
    ...
    const childText = getTextFromReactNode((children.props as any)?.children);
    // If this element has children and any child matches, keep the element with filtered children
    if ((children.props as any)?.children) {
      const filteredSub = filterChildren((children.props as any).children, search);
      if (filteredSub) {
...
export const Combobox: React.FC<ComboboxProps> = ({ label, children, disabled = false }) => {
  ...
  if (open && debouncedSearch) {
    filteredChildren = filterChildren(children, debouncedSearch);
  }
