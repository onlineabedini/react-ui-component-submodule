"use client";
// MultiDatePicker: Custom date picker for multiple date selection in booking sheets
...
interface MultiDatePickerProps {
  value: string[];
  onChange: (dates: string[], availablePeriods?: string[]) => void;
...
const MultiDatePicker: React.FC<MultiDatePickerProps> = ({
  ...
  maxDates = 5, // Default max 5 dates
}) => {
  ...
  const handleDateSelect = (date: Date) => {
    ...
    if (isDateSelected(date)) {
      ...
      if (selectedDates.length >= maxDates) {
        // Show toast or alert for max dates reached
        return;
      }
    ...
    const dateStrings = newSelectedDates.map(d => format(d, 'yyyy-MM-dd'));
    
    // Get available periods for the first selected date (or empty array if none selected)
    const availablePeriods = newSelectedDates.length > 0 
      ? getAvailablePeriodsForDate(newSelectedDates[0])
      : [];

    onChange(dateStrings, availablePeriods);
  };
  ...
  const removeSelectedDate = (dateToRemove: string) => {
    const newSelectedDates = selectedDates.filter(date => 
      format(date, 'yyyy-MM-dd') !== dateToRemove
    );
    setSelectedDates(newSelectedDates);
    const dateStrings = newSelectedDates.map(d => format(d, 'yyyy-MM-dd'));
    const availablePeriods = newSelectedDates.length > 0 
            ...
            {generateCalendarDays().map((date, index) => {
              ...
              return (
                ...
                >
                  {format(date, 'd')}
                  {isSelected && (
                    <div className="absolute -top-1 -right-1 w-5 h-5 bg-teal-600 text-white rounded-full flex items-center justify-center text-xs">
                      ✓
                    </div>
                  )}
