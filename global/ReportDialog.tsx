"use client";
// ReportDialog: Dialog for creating, viewing, and accepting service reports
...
interface Booking {
  ...
  clientId: string;
  providerId: string;
  bookingDate: string[]; // Changed from string to string[] for multiple dates
                ...
                <div className="flex items-center mb-2">
                  <div className="flex-1 flex flex-col items-center bg-white border border-blue-100 rounded-xl p-4 shadow-sm">
                    <span className="flex items-center gap-2 text-blue-700 font-semibold text-base mb-1">
                      <Calendar className="w-5 h-5 text-blue-500" /> {booking.bookingDate.length > 1 ? t('serviceReport.bookingDates') || 'Booking Dates' : t('serviceReport.bookingDate')}
                    </span>
                    <div className="space-y-1">
                      {booking.bookingDate.map((date, index) => (
                        <span key={index} className="text-base font-bold text-blue-900">
                          {/* Use global YYYY-MM-DD date format */}
            ...
            <div className="space-y-6">
              <div className="space-y-4 p-6 bg-gradient-to-br from-teal-50 to-white rounded-xl border border-teal-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  {booking?.bookingDate && booking.bookingDate.length > 0 && (
                    <div className="bg-white p-4 rounded-lg border border-teal-100">
                      <span className="text-sm font-medium text-teal-700">{booking.bookingDate.length > 1 ? t('serviceReport.serviceDates') || 'Service Dates' : t('serviceReport.serviceDate')}</span>
                      <div className="space-y-1">
                        {booking.bookingDate.map((date, index) => (
                          <p key={index} className="text-lg font-semibold text-teal-900">
                            {/* Use global YYYY-MM-DD date format */}
                            {formatDateYYYYMMDD(new Date(date))}
                          </p>
                        ))}
